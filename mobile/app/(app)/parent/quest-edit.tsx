...
import { questService, Quest, LibraryQuest, CreateQuestData } from '../../../src/services/quest';
...
const STACKING_OPTIONS = [
  { value: 'stackable', label: 'Stackable', desc: 'Time stacks with other rewards' },
  { value: 'non_stackable', label: 'Today Only', desc: 'Can only earn once per day' },
];
...
export default function QuestEditScreen() {
  ...
  const [selectedLibraryQuest, setSelectedLibraryQuest] = useState<LibraryQuest | null>(null);

  // Load children for assignment picker
  useEffect(() => {
    if (!familyId) return;
  ...
  const handleAddFromLibrary = async (lq: LibraryQuest) => {
    ...
    if (selectedChildIds.length === 0) {
      Alert.alert('Assign Children', 'Please select at least one child first', [{ text: 'OK' }]);
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>{isEditMode ? 'Edit Quest' : 'New Quest'}</Text>
      ...
      {!isEditMode && (
        <View style={styles.tabRow}>
          {(['custom', 'library'] as CreateTab[]).map((tab) => (
            ...
            >
              <Text style={[styles.tabText, createTab === tab && styles.tabTextActive]}>
      ...
      {createTab === 'library' && !isEditMode ? (
        <ScrollView style={styles.scrollView} contentContainerStyle={styles.scrollContent}>
          ...
          <View style={styles.childRow}>
            {children.length === 0 ? (
              <Text style={styles.noChildren}>No children added to your family yet</Text>
            ) : (
              children.map((child) => (
                <TouchableOpacity
                  key={child.id}
                  style={[styles.childChip, selectedChildIds.includes(child.id) && styles.childChipSelected]}
                  onPress={() => toggleChild(child.id)}
                >
                  <Text style={[styles.childChipText, selectedChildIds.includes(child.id) && styles.childChipTextSelected]}>
                    {child.name}
                  </Text>
                </TouchableOpacity>
              ))
          ...
          <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.catScroll}>
            ...
            >
              <Text style={[styles.catChipText, !libraryCategory && styles.catChipTextActive]}>All</Text>
            </TouchableOpacity>
            {CATEGORIES.filter((c) => c.value !== 'custom').map((cat) => (
              <TouchableOpacity
                key={cat.value}
                style={[styles.catChip, libraryCategory === cat.value && styles.catChipActive]}
                onPress={() => setLibraryCategory(cat.value)}
              >
                <Text style={[styles.catChipText, libraryCategory === cat.value && styles.catChipTextActive]}>
                  {cat.icon} {cat.label}
                </Text>
              </TouchableOpacity>
            ))}
            ...
            libraryQuests.map((lq) => (
              ...
              >
                ...
                <View style={styles.libraryInfo}>
                  ...
                  {lq.description && (
                    <Text style={styles.libraryDesc} numberOfLines={2}>{lq.description}</Text>
                  )}
                  <View style={styles.libraryMeta}>
                    <Text style={styles.libraryReward}>{lq.suggestedRewardMinutes} min</Text>
                    {lq.ageRange && <Text style={styles.libraryAge}>{lq.ageRange.replace('_', ' ')}</Text>}
                  </View>
                </View>
        ...
        <ScrollView style={styles.scrollView} contentContainerStyle={styles.scrollContent}>
          ...
          <View style={styles.nameRow}>
            <TouchableOpacity style={styles.iconPicker} onPress={() => setShowEmojiPicker(!showEmojiPicker)}>
          ...
          {showEmojiPicker && (
            <View style={styles.emojiGrid}>
              {EMOJI_OPTIONS.map((emoji) => (
                <TouchableOpacity
                  key={emoji}
                  style={[styles.emojiBtn, icon === emoji && styles.emojiBtnSelected]}
          ...
          <View style={styles.optionGrid}>
            {CATEGORIES.map((cat) => (
              ...
              >
                ...
                <Text style={[styles.optionText, category === cat.value && styles.optionTextActive]}>
                  {cat.label}
                </Text>
              </TouchableOpacity>
            ))}
          ...
          <View style={styles.rewardRow}>
            {REWARD_PRESETS.map((mins) => (
              <TouchableOpacity
                key={mins}
                style={[styles.rewardChip, rewardMinutes === mins && styles.rewardChipActive]}
                onPress={() => setRewardMinutes(mins)}
              >
                <Text style={[styles.rewardChipText, rewardMinutes === mins && styles.rewardChipTextActive]}>
                  {mins}m
                </Text>
              </TouchableOpacity>
            ))}
          ...
          <View style={styles.stackRow}>
            {STACKING_OPTIONS.map((opt) => (
              <TouchableOpacity
                key={opt.value}
                style={[styles.stackCard, stackingType === opt.value && styles.stackCardActive]}
                onPress={() => setStackingType(opt.value)}
              >
                <Text style={[styles.stackLabel, stackingType === opt.value && styles.stackLabelActive]}>
                  {opt.label}
                </Text>
          ...
          <View style={styles.rewardRow}>
            {RECURRENCE_OPTIONS.map((opt) => (
              <TouchableOpacity
                key={opt.value}
                style={[styles.rewardChip, recurrence === opt.value && styles.rewardChipActive]}
                onPress={() => setRecurrence(opt.value)}
              >
                <Text style={[styles.rewardChipText, recurrence === opt.value && styles.rewardChipTextActive]}>
                  {opt.label}
                </Text>
              </TouchableOpacity>
            ))}
          ...
          <View style={styles.toggleRow}>
            <View style={styles.toggleInfo}>
              ...
              <Text style={styles.toggleDesc}>Automatically approve completions</Text>
            </View>
            <Switch
              value={autoApprove}
...
const styles = StyleSheet.create({
  ...
  rewardChipActive: { backgroundColor: colors.primary, borderColor: colors.primary },
  rewardChipText: { fontSize: 14, fontWeight: '600', color: colors.textSecondary },
  ...
  stackLabel: { fontSize: 15, fontWeight: '700', color: colors.textPrimary, marginBottom: 2 },
  ...
  childChipSelected: { borderColor: colors.secondary, backgroundColor: '#E8F5E9' },
  childChipText: { fontSize: 14, fontWeight: '600', color: colors.textSecondary },
  childChipTextSelected: { color: '#2E7D32' },
  noChildren: { fontSize: 14, color: colors.textSecondary, fontStyle: 'italic' },
  toggleRow: {
    flexDirection: 'row',
    alignItems: 'center',
    backgroundColor: colors.card,
  ...
  libraryMeta: { flexDirection: 'row', alignItems: 'center', gap: spacing.sm, marginTop: 4 },
  libraryReward: { fontSize: 13, fontWeight: '600', color: colors.primary },
  libraryAge: { fontSize: 11, color: colors.textSecondary, backgroundColor: '#F5F5F5', paddingHorizontal: 6, paddingVertical: 2, borderRadius: 8 },
});
