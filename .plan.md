...
- **CI/CD**: No GitLab CI/CD pipelines
...
### Step 10: CI/CD GitLab Pipelines

- `.gitlab-ci.yml` with stages: `backend-ci` (lint, type-check, unit tests, integration tests with PostgreSQL + Redis services, build, coverage upload), `mobile-ci` (lint, type-check, EAS build preview profile), `cms-ci` (lint, type-check, build)
- Triggered on MR to main + push to main
